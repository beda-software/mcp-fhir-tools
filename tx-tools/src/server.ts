...
export default function createServer(): McpServer {
  ...
  server.tool(
    ...
    `,
    {
      ...
      txServer: z.string().optional()
        .describe(`Optional URL of the terminology server to use for validation.
    ...
    async ({ filter, url, txServer }) => {
      ...
      try {
        ...
        return {
          content: [
            { type: "text", text: JSON.stringify(firstMatch, null, 2) },
          ],
        };
      } catch (error) {
        return {
